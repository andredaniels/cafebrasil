...
Tags: affiliate, cart, checkout, ecommerce, online, paypal, physical, reports, sagepay, moneybookers, sales, sell, shipping, shop, shopping, stock, store, tax, virtual, weights, widgets, wordpress ecommerce, commerce, e-commerce, reports, online shop, shopping, online store, authorize, cart, stock control, tax, wp e-commerce, 2checkout, 2co, coupons, gifts, ready!, paypal pro, paypal advanced, paypal express
Tested up to: 4.0
Stable tag: 0.5.3

Shopping cart / eCommerce WP plugin. Easy to setup online store. Powerful functionality. Include free e-commerce themes.
...
[Ready! eCommerce Wordpress Plugin](http://readyshoppingcart.com/ecommerce-plugin/ "eCommerce Wordpress Plugin") with Shopping Cart online store v0.5.2 include:
...
== Changelog ==
= 0.5.3 =
 * Fix for related products table load
 * Minor issues fix
 * Code improve

= 0.5.2 =
 * Watermarks - fixed bug with uploading images - http://readyshoppingcart.com/product/watermark-product-images-extension/
 * Some optimizations for framework load and work
 * Additional security improvements
 * Do not load jquery tabs css in all admin area
 * Additional orders processing before output in admin area
 * Remove timthumb.php script

= 0.5.1 =
 * Fix for correct orders export button display
 * Some security checkout issues fix
 * Minor fixes

= 0.5.0 =
 * Do not show shipping method section if user order only downloadable products
 * USPS - fix for Commercial rates - https://readyshoppingcart.com/product/usps-shipping-module/
 * Do not show email user field in admin area -> user profile second time (first time it is shown by wp)
 * Small fixes for related products view in admin area
 * Make Downloads inputs in admin area - more user-friendly, added tips for this section

= 0.4.9 =
 * Start session only in case it were not started before (make additional check)
 * Small fix for addons activation check - https://readyshoppingcart.com/extensions/
 * Code imporvements

= 0.4.8 =
 * Fix of currency view setup
 * Small fix for displaying pages in menus
 * Code improvements

= 0.4.7 =
 * Fix issue when some state was removed
 * Small code review

= 0.4.6 =
 * GlobalPay payment addon - https://readyshoppingcart.com/product/globalpay-payment-gateway-plugin/
 * InternetSecure payment addon - https://readyshoppingcart.com/product/internetsecure-wordpress-payment-gateway/
 * Redsys payment addon - https://readyshoppingcart.com/product/redsys-payment-gateway-wordpress-plugin/
 * Payza payment addon - https://readyshoppingcart.com/product/payza-payment-gateway-for-wordpress/
 * Realex - fix order ID for realex gateway (it should be random)
 * Gifts - fix remove product from cart and update gifts conditions - https://readyshoppingcart.com/product/special-products-gifts-coupons-pack/

= 0.4.5 =
 * Fix pages re-creation from admin area (for case them was deleted)
 * Shipping tablerate - base your tablerate not only on weight, but also on price or QTY
 * Small code review and imporvements

= 0.4.4 =
 * Fix default plugin styles filenames detect in templates
 * Minor fix for gifts qty detection http://readyshoppingcart.com/product/special-products-gifts-coupons-pack/

= 0.4.3 =
 * More correct welcome page display condition
 * Fix problems with gifts whern products qty is changed on shopping cart page - http://readyshoppingcart.com/product/special-products-gifts-coupons-pack/

= 0.4.2 =
 * Main framework router - fix incompatibility for promo module with our other plugins
 * JA language
 * Placeholder js - fix for cases when lib were not loaded
 * Catalog addon - improve of interface, small fixes - http://readyshoppingcart.com/product/catalog_from_the_onlinestore/
 * Main stiles load - fix handle names, make them more unique, to avoid incompatibility with other such style names
 * Welcome page data - submit with usual GET request, not ajax
 * Some spelling issues fix

 = 0.4.1 =
 * Posibility to change variation without reload - only substitute product ID in current product page
 * Coupons - use applicable price as price for selected product if they was selected
 * Realex payment - http://readyshoppingcart.com/product/realex-payments-plugin/
 * Tax rates - possibility to apply taxes for categories of products
 * Free shipping for digital products - exclude additional product files from addon http://readyshoppingcart.com/product/product-additional-files/
 * SSL detection - use build-in wordpress functionality
 * Minor bugs fix

 = 0.4.0.9 =
 * Removed some deprecated code
 * Admin tab styles - make them resizable
 * Intuit Schedule - fix instalation warning
 * Shipping address on checkout - resolve issue with some characters
 * Tax rate - include shipping (if it should be included) for PayPal Standard
 * Product additional fields - fix for checkbox list fields type
 * Product additional fields - fix display of selected values for checkbox, radiobutton and selectbox types
 * Orders sorting - desc by default, added "All" as number to show option
 * Gifts addon - if present is product - show only available products for selection (http://readyshoppingcart.com/product/special-products-gifts-coupons-pack/)
 * Do not hide unused shipping modules on checkout when select shipping module
 * Fix copy billing state to shipping state on checkout if shipping info is same as billing
 * Fix for sub-screen hiding in top when show in admin area
 * Standard wordpress submit buttons for admin area
 * Some minor bug fixes

 = 0.3.5.2 =
 * Shipping per item - fix to include QTY when calculating
 * Options tabs - fix for tabs, that have default view to show content
 * Some wording fix
 * Product additional fields - some fix for correct detecting categories and products in corresponding lists
 * Checkout additional shipping options - correct update each time after reload
 * Digital products - fix for correct files path
 * Product Downloads - remote files upload fix

 = 0.3.5.1 =
 * Hot fix for "bad" space in code

 = 0.3.5.0 =
 * Add features - add/remove product to/from order 
 * New products Fields functionality
 * Add options to add/edit files, download limit, period to files 
 * Welcome page data - more interactions with users
 * More flexible database update system - make all update process more faster
 * Wording corrections
 * Changes in orders management
 * Changes in Dummy Data module
 * Some code improvements
 * Some security additional fixes

 = 0.3.4.9 =
 * Products extra fields - hot fix

 = 0.3.4.8 =
 * Orders list userfriendly manipulations 
 * Intuit Scheduled Payment integration
 * Small additional fields fix
 * API - Cart Update method
 * API - Added possibility to load and update checkout cart data via ajax
 * Translator - nl_NL language added
 * Style for "Add to cart" button in latest wordpress themes (padding fixing)
 * 2checkout - modification to make checkout more easy for customers
 * Paymentech payment gateway integration
 * Products Extra fields - small fix
 * More interactions with users - plugin Welcome page
 * More interactions with users - Plugin modules usage statistics
 * Small wording issues fix
